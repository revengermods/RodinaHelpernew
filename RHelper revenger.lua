...
function main()
	...
	act = not act; sampAddChatMessage(act and '{00C091}[Rodina Helper]{FFFFFF} Реклама {00C091}включена!' or '{00C091}[Rodina Helper]{FFFFFF} Реклама {E32636}выключена!', -1)
		if act then
    		 piar()
   		 end
	end)
    ...
    downloadUrlToFile(update_url, update_path, function(id, status)
        if status == dlstatus.STATUS_ENDDOWNLOADDATA then
            ...
            if tonumber(updateIni.info.vers) > script_vers then
                sampAddChatMessage("{00C091}[Rodina Helper] {ffffff}Есть обновление! Версия: " .. updateIni.info.vers_text, -1)
                update_state = true
            end
	...
	-----------------------------------------------
	sampAddChatMessage("{00C091}[Rodina Helper]{FFFFFF} Успешно загружен! Активация: {00C091}/rhelp",-1)
	sampAddChatMessage("{00C091}[Rodina Helper]{FFFFFF} Если у вас нет ID цветов, то обратитесь к {00C091}[vk.com/qweeqwz]", -1)
    ...
    while true do
	  ...
	  if update_state then
		downloadUrlToFile(script_url, script_path, function(id, status)
			if status == dlstatus.STATUS_ENDDOWNLOADDATA then
				sampAddChatMessage('{00C091}[Rodina Helper] {ffffff}Скрипт успешно обновлен! | Связь: {00C091}[vk.com/qweeqwz]', -1)
				thisScript():reload()
			end
		end)
		break
